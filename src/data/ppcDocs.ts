...
/**
 * `지식/marketing/ppc/` 트리를 빌드 타임에 읽어 WikiDoc[]로 변환한다.
 * 파일 하나 = 문서 하나. 프론트매터 규격은 `지식/README.md` 참고.
 */
const files = import.meta.glob('/지식/marketing/ppc/**/*.md', {
  query: '?raw',
  import: 'default',
  eager: true,
...
const parsed: ParsedDoc[] = Object.entries(files)
  .map(([filePath, raw]): ParsedDoc | null => {
    ...
    return {
      ...
      path,
      title: fm.title ?? path,
      category: (fm.category as WikiDoc['category']) || 'marketing',
      code: fm.code,
      body,
...
export const ppcDocs: WikiDoc[] = flattenTree(parsed)
